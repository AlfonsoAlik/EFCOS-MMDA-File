...
df = pd.read_csv('Flood_report_rain_no_rain_3200.csv')

column = ['RAINFALL', 'TMAX', 'TMIN', 'TMEAN', 'WIND_SPEED', 'WIND_DIRECTION', 'ELEVATION', 'SLOPE']
...
def user_input_features():
    ...
    ELEVATION = st.sidebar.number_input('Elevation', min_value=float(X.ELEVATION.min()),value=float(X.ELEVATION.mean()))
    SLOPE = st.sidebar.number_input('Slope',min_value=float(X.SLOPE.min()),value=float(X.SLOPE.mean()))
